...
- ChineseLogicQA: A Chinese logical reasoning benchmark collected from civil servant/standardized exams. The repository ships a pre-processed JSONL file at `data/chinese-logicqa/test_zh.txt`.
...
python gpt3_baseline.py \
    ...
    --dataset_name "Dataset Name [ProntoQA | ProofWriter | FOLIO | LogicalDeduction ｜ AR-LSAT ｜ chinese-logicqa]" \
    --split dev \
    --mode "Baseline [Direct | CoT]" \
    --max_new_tokens "16 for Direct; 1024 for CoT" \
    --api_base_url "Optional custom endpoint such as https://api.deepseek.com/v1" \
```

When evaluating `chinese-logicqa`, set `--split test_zh` (the loader automatically reads `data/chinese-logicqa/test_zh.txt`).
...
```bash
DATASET="Dataset Name [ProntoQA | ProofWriter | FOLIO | LogicalDeduction ｜ AR-LSAT ｜ chinese-logicqa]"
...
> Note: set `SPLIT=test_zh` when running on `chinese-logicqa`.
...
python models/evaluation.py \
    --dataset_name "Dataset Name [ProntoQA | ProofWriter | FOLIO | LogicalDeduction ｜ chinese-logicqa]" \
...
baseline
python gpt3_baseline.py --api_key sk-81c7577b681e483dbbb2b5e466db3688 --model_name deepseek-chat --dataset_name LogicalDeduction --split dev --mode Direct --max_new_tokens 3000 --api_base_url "https://api.deepseek.com/v1"

LogicalDeduction AR-LSAT ProofWriter 

python evaluation.py --dataset_name "FOLIO" --model_name "deepseek-chat" --split dev  --mode "Direct"

logiclm
python models/logic_program.py --api_key sk-81c7577b681e483dbbb2b5e466db3688 --dataset_name chinese-logicqa --split test_zh_300 --model_name deepseek-chat --api_base_url "https://api.deepseek.com/v1"
python models/logic_inference.py --dataset_name chinese-logicqa --split test_zh --model_name deepseek-chat --backup_strategy random



python models/evaluation.py \
    --dataset_name "chinese-logicqa" \
    --model_name "deepseek-chat" \
    --split test_zh_300 \
    --backup "random"


python analyze_reasoning_tokens.py baselines/results/Direct_chinese-logicqa_test_zh_300_deepseek-chat.json --no_tiktoken